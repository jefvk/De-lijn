               ...
               if (err) {
                 ...
                 request.get({url:getCoorNaar, json:true}, (err,res,data)=>{
                   if (err) {
                     ...
                     console.log(yVan);
                     console.log(xVan);
                     var finalUrl = 'https://www.delijn.be/rise-api-core/reisadvies/routes/'+ req.body.Van + '/' +req.body.Naar +'/'+xVan+'/'+yVan+'/'+xNaar+'/'+yNaar+'/20-12-2017/!:00/1/on/of/of/of/of/nl';
                     request.get({url:finalUrl, json:true}, (err,res,data)=>{
                       if (err) {
                         console.log(err);
                       }else if (res.statusCode === 200){
                         console.log(data);
                         start = data.reiswegen[0].startTime;
                         console.log(start);
